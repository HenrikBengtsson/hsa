...
loglikelihood <- function(P0, A, b, invSigma, beta, cx, mat, pos=NULL, v=NULL, mak=NULL) {
  ...
  if (is.null(mak)) {
    L = L + sum(sapply(2:N, function(ii) {
      nmp = fmkorder(P0[ii, 1] - P0[ii - 1, 1], A, b, sigma, P[ii - 1, ]);-(P[ii, ] - nmp[, 1]) %*% (nmp[, 2:4]) %*% (P[ii, ] - nmp[, 1]) / 2 + log(det(nmp[, 2:4])) / 2
    }) / N / 3)
  } else {
    L = L + sum(sapply(2:N, function(ii) {
      mu = mak[[ii - 1]][, 1] + mak[[ii - 1]][, 5:7] %*% P[ii - 1, ];-t(P[ii, ] - mu) %*% mak[[ii - 1]][, 2:4] %*% (P[ii, ] - mu) / 2 + log(det(mak[[ii - 1]][, 2:4])) / 2
    })) / N / 3
  }
  L
}
...
dloglikelihood <- function(P0, A, b, invSigma, beta, cx, mat, pos, v=NULL, mak=NULL) {
  ...
  if (is.null(mak)) {
    ...
    temp1 = sapply(2:N, function(ii) {
      mu = mak[[ii - 1]][, 1] + mak[[ii - 1]][, 5:7] %*% P[ii - 1, ];-t(P[ii, ] - mu) %*% mak[[ii - 1]][, 2:4]
    }) / N / 3
    dL[2:N, ] = dL[2:N, ] + t(temp1)
