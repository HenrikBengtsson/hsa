...
fmain<-function(lsmap0,lscov0,output,Maxiter,submaxiter,lamda,Leapfrog,epslon,mkfix=0,rho=0,mk,initialS=NULL,coarsefit=TRUE,rmoutlier=FALSE,fitmode=0)
{
    ...
    recodllk=rep(0,7)
    if(mk){floglike=loglikelihood ##if Markov then different likehood functions called, minus 0s
    ...
    mat=array(NA,c(N,N+1,C))##3-dimensional array of N by N+1 by C all filled with NAs
    mat0=mat ##Same NA matrix as mat
    beta1=-rep(1.3,C) ##the number 1.3 repeated C times
    alpha=seq(0.5,1.5,0.5) ##the squence 0.5, 1, 1.5
    covmat0=array(1,c(N,N,C)) ##a N by N by C matrix of 1s
	...
	for (c in 1:C){ ##loop through contact matrix loops similar, perhaps exactly the same
        ...
        covmat0=array(1,c(N,N,C)) ##covmat0 re-written as 3-dimensional array of 1s
    ...
    v=lapply(1:C,function(i) which(mat[pos[[i]],pos[[i]]+1,i]>0)) ##list of positions greater than 0 between adjacent bins
    if(mk){ ##if Markov property
        mak=lapply(2:N, function(ii) fmkorder2(pbin[ii]-pbin[ii-1],A0,b0,solve(invSigma0))) ##Update Markov matrix
	...
	index2=c(index2,N)	
	Psub=P01
	Psub=do.call(rbind,lapply(1:dim(index)[1],function(x) tranS(suboptimz(pbin[index[x,1]:index[x,2]],P01[index[x,1]:index[x,2],],A0,b0,invSigma0,beta1,covmat0[index[x,1]:index[x,2],index[x,1]:index[x,2],],mat[index[x,1]:index[x,2],c(1,index[x,1]:index[x,2]+1),],floglike,fdloglike,0),P01[index[x,1]:index[x,2],]))) ##complex rbind
	...
	while(u<submaxiter && (N<1000 || (iternum%%10==1 && iternum>5) ||u<3)){	##likelihood optimization not totally understood		
        ...
        if(!mk){P=normP(P)} ##If not Markov property normalize P
